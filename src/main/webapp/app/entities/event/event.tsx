  ...
  return (
    <div>
      <h2 id="event-heading" data-cy="EventHeading">
        <Translate contentKey="rollinHungerApp.event.home.title">Events</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="rollinHungerApp.event.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/event/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="rollinHungerApp.event.home.createLabel">Create new Event</Translate>
          </Link>
        </div>
      </h2>
        ...
        {eventList && eventList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="rollinHungerApp.event.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="rollinHungerApp.event.location">Location</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {eventList.map((event, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{event.location ? <Link to={`/location/${event.location.id}`}>{event.location.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button tag={Link} to={`/event/${event.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="rollinHungerApp.event.home.notFound">No Events found</Translate>
            </div>
          )
        )}
      </div>
