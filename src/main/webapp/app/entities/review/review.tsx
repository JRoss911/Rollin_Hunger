  ...
  return (
    <div>
      <h2 id="review-heading" data-cy="ReviewHeading">
        <Translate contentKey="rollinHungerApp.review.home.title">Reviews</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="rollinHungerApp.review.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/review/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="rollinHungerApp.review.home.createLabel">Create new Review</Translate>
          </Link>
        </div>
      </h2>
        ...
        {reviewList && reviewList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="rollinHungerApp.review.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="rollinHungerApp.review.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {reviewList.map((review, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{review.date ? <TextFormat type="date" value={review.date} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{review.user ? <Link to={`/user-profile/${review.user.id}`}>{review.user.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="rollinHungerApp.review.home.notFound">No Reviews found</Translate>
            </div>
          )
        )}
      </div>
